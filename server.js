...
// Carga opcional de .env local (sin dependencia extra)
(() => {
  try {
...
function normalize(body) {
  ...
  return {
    id: body?.id || `brief_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
...
app.get("/api/export.csv", (_req, res) => {
  ...
  res.setHeader("Content-Disposition", 'attachment; filename="brief-mercadotecnia-respuestas.csv"');
  res.send("\uFEFF" + lines.join("\n"));
});
