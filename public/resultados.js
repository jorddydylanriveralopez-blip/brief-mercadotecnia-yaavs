(() => {
  ...
  const LABELS = [
    ...
    ["fechaPropuesta", "Fecha 1ª propuesta"],
    ["fechaFinal", "Fecha entrega final"],
  ...
  function countBy(list, key) {
    const map = new Map();
    list.forEach((r) => {
      const raw = String(r[key] || "").trim() || "Sin dato";
  ...
  function renderDetail() {
    ...
    const fields = LABELS.map(([key, label]) => {
      const v = val(r[key]);
      if (v.na && (key.endsWith("Otra") || key === "coberturaDetalle" || key.startsWith("oferta") || key === "textoListo" || key === "lineaGrafica" || key === "comentarios" || key === "fechaPublicacion" || key === "entregableOtro" || key === "canalOtro" || key === "servicioOtro" || key === "objetivoOtro" || key === "marcaOtra" || key === "areaOtra")) {
        return "";
      }
      return `
        <div class="field">
